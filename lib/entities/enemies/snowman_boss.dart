...
class SnowmanBoss extends EnemyCharacter {
  SnowmanBoss(double x, double y)
      : super(
          static: false,
          removeOnDeath: true,
    ...
    characterAnimation = SnowmanAnimation();
  ...
  SnowmanBoss.fromTiledObject(TiledObject object) : this(object.x, object.y);
  ...
  void onDeath() {
    ...
    statuses.add(DeadSnowmanStatus());
...
/// this to grow/shrink the Snowman
class _AnimationWrapper extends PositionComponent
    with HasAncestor<SnowmanBoss> {
...
class SnowmanAnimation
    extends CharacterAnimation<_AnimationState, SnowmanBoss> {
  @override
  @mustCallSuper
...
class DeadSnowmanStatus extends StatusComponent with IgnoredByWorld {}

class SnowmanBossTrigger extends PhysicalEntity {
  SnowmanBossTrigger(TiledObject object) : super(static: true) {
    x = object.x;
    y = object.y;
    width = object.width;
  ...
  void trigger() {
    map.add(SnowmanBoss(right + map.tileSize * 5, bottom - map.tileSize * 3));
    FlameAudio.bgm.play('broken-bitz.mp3');
    removeFromParent();
  }
}

class SnowmanBossFactory implements TiledObjectHandler {
  SnowmanBossFactory();

  @override
  void handleObject(TiledObject object, Layer layer, LeapMap map) {
    map.add(SnowmanBoss.fromTiledObject(object));
  }
}

class SnowmanBossTriggerFactory implements TiledObjectHandler {
  SnowmanBossTriggerFactory();

  @override
  void handleObject(TiledObject object, Layer layer, LeapMap map) {
    map.add(SnowmanBossTrigger(object));
  }
}
